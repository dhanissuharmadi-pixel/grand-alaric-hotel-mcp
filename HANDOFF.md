...
- ✅ Full in-widget booking flow: search → hotel list → details → dates → rooms → enhance → guest → pay.
- ✅ **Tested live in ChatGPT.** The unified in-widget flow works — `callTool` returns data to the
  calling widget without swapping templates (the "callTool assumption" held).
- ✅ **Cart-based booking + multi-room** wired to the new `/orders` contract (see "API shapes").
- ✅ Light + dark mode both render correctly (see "Theme / background").
- 🔧 **Two backend/config issues block polish** (not our code): broken `hotel_cover` images and an
